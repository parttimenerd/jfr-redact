#!/usr/bin/env python3
"""
Sync documentation script for jfr-redact.
...
def extract_version_from_java() -> str:
    ...
    version_file = get_repo_root() / "src/main/java/me/bechberger/jfrredact/Version.java"
...
def update_pom_version(version: str, dry_run: bool) -> bool:
    ...
    # Pattern: <version>...</version> that follows <artifactId>jfr-redact</artifactId>
    new_content = re.sub(
        r'(<artifactId>jfr-redact</artifactId>\s*<version>)[^<]+(</version>)',
        rf'\g<1>{version}\g<2>',
        content,
        count=1
    )
...
def build_project_and_get_help() -> dict[str, str]:
    ...
    jar_path = repo_root / "target/jfr-redact.jar"
...
# Pre-commit hook for jfr-redact
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync documentation for jfr-redact",
